        ...
        if latest_msg.get("role") == "tool":
            ...
            if "Traceback" in content or (debug_filename and debug_filename in action):
                ...
                if guidance_prompt:
                    ...
                    messages_copy.append({"role": "user", "content": guidance_prompt, "message_type": "guidance"})

                    # Mark as called to prevent future injections
                    self.enhance_history_dict[instance_id]["claude_analysis"]["is_called"] = (
...
def load_enhancement_data(json_file_path: str) -> bool:
    ...
    try:
        ...
        if IntelligentGuidanceHook.enhance_history_dict is None:
            ...
            logger.info(
                f"Successfully loaded enhancement data for {len(IntelligentGuidanceHook.enhance_history_dict)} instances"
            )
        else:
