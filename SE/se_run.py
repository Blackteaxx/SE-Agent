...
def validate_config(config_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        if not isinstance(instance_list, list) or len(instance_list) == 0:
            raise ConfigValidationError("实例列表为空或格式错误")
...
示例用法:
  ...
  %(prog)s --config config.yaml --clean-restart
    ...
    parser.add_argument("--config", default="SE/configs/se_configs/test_deepseek_se.yaml", help="SE配置文件路径")
    parser.add_argument(
        "--mode", choices=["demo", "execute"], default="execute", help="运行模式: demo=演示模式, execute=直接执行"
    )
    parser.add_argument("--resume", action="store_true", help="恢复模式: 从上次中断处继续执行")
    parser.add_argument("--clean-restart", action="store_true", help="清理重启: 删除现有工作空间重新开始")
    parser.add_argument("--validate-only", action="store_true", help="仅验证配置文件，不执行")
    ...
    try:
        ...
        # 从 SE 配置传入并发数以控制轨迹总结的线程池大小
        traj_pool_manager = TrajPoolManager(traj_pool_path, llm_client, num_workers=config.get("num_workers"))
                    ...
                    try:
                        ...
                        if tra_stats and tra_stats.get("total_tra_files", 0) > 0:
                            ...
                            try:
                                ...
                                logger.info(
                                    f"轨迹池更新完成: {pool_stats['total_instances']}实例, "
                                    f"{pool_stats['total_iterations']}总迭代"
                                )
                                print(
                                    f"🏊 轨迹池更新: {pool_stats['total_instances']}实例, "
                                    f"{pool_stats['total_iterations']}总迭代"
                                )
        ...
        print(f"  ✅ 日志文件: {log_file}")
        print(f"  🏊 轨迹池: {final_pool_stats['total_instances']}实例, {final_pool_stats['total_iterations']}总迭代")
