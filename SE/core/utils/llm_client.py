...
class LLMClient:
    ...
    def __init__(self, model_config: dict[str, Any]):
        ...
        # 请求控制参数（带默认值）
        self.request_timeout: float = float(self.config.get("request_timeout", 300.0))
    ...
    def call_llm(
        ...
        Args:
            ...
            enable_thinking: 是否启用思考模式（通过 extra_body.chat_template_kwargs 传递）
        ...
        while attempt < self.max_retries:
            try:
                ...
                # 可选：传递 enable_thinking 到 extra_body
                if enable_thinking is not None:
                    create_kwargs["extra_body"] = {"chat_template_kwargs": {"enable_thinking": bool(enable_thinking)}}
