...
class TrajExtractor:
    ...
    def _load_perf_metrics(self, instance_dir: Path) -> Optional[Dict[str, Any]]:
        """
    ...
    def extract_instance_data(self, iteration_dir: str, include_metrics: bool = False) -> List[Tuple[str, Optional[str], str, str]]:
        """
        从迭代目录中提取所有实例的数据
        ...
        for instance in instances:
            if instance.tra_content:
                ...
                if include_metrics:
                    # 尝试读取性能指标（来自PerfAgent输出的result.json）
                    perf_metrics = None
                    try:
                        perf_metrics = self._load_perf_metrics(instance.instance_path)
                    except Exception as e:
                        self.logger.debug(f"读取性能指标异常: {instance.instance_name}: {e}")
                    results.append((
