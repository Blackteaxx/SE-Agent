...
def call_operator(operator_name, workspace_dir, current_iteration, se_config, logger):
    ...
    Returns:
        operator返回的参数字典 (如 {'initial_code_dir': 'path'}) 或 None表示失败
    """
    try:
...
def call_perfagent(iteration_params, logger, dry_run=False):
    ...
    try:
        ...
        cmd.extend([
            "--instances-dir", str(instances_dir),
            "--base-dir", str(base_dir),
            "--max-workers", str(iteration_params.get("num_workers", 1)),
        ])

        # 若 operator 返回 initial_code_dir，则传给 run_batch 供 PerfAgent 注入初始代码
